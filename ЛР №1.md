<h1>ЗВІТ ПО ВИКОНАННЮ ЛАБОРАТОРНОЇ РОБОТИ №1</h1>
...
Перевірила викладач Сушанова В.С. 
...
1. \*Прочитайте короткі теоретичні відомості до лабораторної роботи та зробіть невеликий словник базових англійських термінів з питань класифікації віртуальних середовищ.<br>  
- **Hypervisor** – software or hardware for running virtual machines <br>
- **Virtual Machine (VM)** – an emulated computer system <br>
- **Guest OS** – operating system running inside the VM <br>
- **Host OS** – operating system of the physical machine <br>
- **Kernel** – the core of an operating system <br>
- **Open source** – source code available for everyone <br>
- **Command Line Interface (CLI)** – text-based interface <br>
- **Graphical User Interface (GUI)** – graphical interface <br>
- **Package Manager** – software tool to install/remove programs <br>
- **Distribution (distro)** – complete Linux system bundle <br>  
...
**Hypervisor** is software (or hardware layer) that enables running virtual machines by managing hardware resources. <br><br>
Types of hypervisors: <br>
- **Type 1 (bare-metal):** runs directly on hardware. <br>
- **Type 2 (hosted):** runs on top of an existing OS. <br>  
   2.2. Перерахуйте основні компоненти та можливості гіпервізорів відповідно до свого варіанту (порядковий номер по журналу), табл.1.<br>  
 VirtualBox — Components and Features (Variant 1)<br>  
- Runs as a **Type 2 hypervisor** on top of host OS. <br>
- Provides both **GUI** and **CLI** (`VBoxManage`). <br>
- Supports various guest OS: Linux, Windows, macOS, BSD. <br>
- Snapshot system for saving/restoring VM states. <br>
- Shared folders and drag-and-drop support. <br>
- Guest Additions for better integration with host. <br>
- Flexible hardware configuration (CPU, RAM, GPU, network, USB). <br>
- Virtual networking support (NAT, Bridged, Host-only). <br>
Таблиця 1
...
(This task does Andrii Berdiaiev)  
   2.4. \*Яким чином можна до установити графічні оболонки Gnome та KDE в Linux, якщо вона вже встановлена в текстовому режимі (вкажіть необхідні команди та пакети)?<br>  
      If Linux was installed in **CLI (text mode)**, you can add GUI as follows: <br>
...
```
sudo apt install gnome-shell gdm3       # Debian/Ubuntu
sudo dnf groupinstall "Server with GUI" # Fedora/RHEL
```
...
sudo dnf groupinstall "KDE Plasma Workspaces" # Fedora/RHEL
```

4. **Enable graphical target and reboot:**
...
## Graphical Interfaces (Variant 1 — KDE & Fluxbox)

### KDE Plasma

* Modern, powerful, highly customizable desktop environment. <br>
* Windows-like UI (taskbar, start menu, system tray). <br>
* Ships with many applications (Konsole, Dolphin, Kate). <br>
* Beautiful design but higher resource consumption. <br>
* Used in **Kubuntu, openSUSE, Fedora KDE Spin**. <br>

### Fluxbox

* Lightweight **window manager**, not a full DE. <br>
* Very fast and minimalistic, suitable for old PCs. <br>
* Configured via text files. <br>
* Often included in lightweight distros like **antiX, TinyCore Linux**. <br>
...
During this laboratory work we explored the basics of virtualization and the Linux operating system in a practical context. The key outcome was understanding how virtualization allows multiple operating systems to run on the same hardware using hypervisors. We analyzed different types of hypervisors and studied VirtualBox (our variant), which represents a Type 2 hypervisor. Its main advantages include cross-platform support, ease of use, snapshot management, and integration with host resources.

Another important aspect of the work was learning about the Linux ecosystem. We revised the fundamental difference between the **CLI (Command Line Interface)** and **GUI (Graphical User Interface)**, and we practiced how to extend a text-only Linux installation with graphical environments such as **GNOME** and **KDE**. This knowledge is crucial because in real-world scenarios system administrators often start with minimal installations and then customize them according to project requirements.

Additionally, we compared graphical interfaces **KDE Plasma** and **Fluxbox**, which represent two different approaches: KDE as a powerful and feature-rich desktop environment, and Fluxbox as a lightweight and highly efficient window manager for older or resource-constrained hardware. This demonstrates the flexibility of Linux distributions in adapting to both high-performance and low-resource use cases.

Through the control questions we also reinforced theoretical knowledge about open-source licensing (GNU GPL), the concept of Linux distributions, embedded Linux applications, and the relationship between Android and Linux. These insights highlight the broad applicability of Linux, from personal desktops to enterprise servers and embedded systems.

In summary, the laboratory work provided both theoretical and practical understanding of:
- how virtualization works and where it is applied,
- the role of hypervisors and their types,
- the installation and configuration of graphical environments in Linux,
- the diversity of Linux desktop environments and their use cases,
- the fundamental principles of open-source software and Linux distributions.

This knowledge lays the foundation for more advanced studies in system administration, virtualization, and operating system management. It also develops practical skills that are directly applicable in IT infrastructure, DevOps practices, and software engineering environments.
